  ...
  return (
    <AppShell>
      <header className="px-5 pt-[max(2.25rem,env(safe-area-inset-top))] pb-3">
        ...
        <h1 className="mt-0.5 text-3xl font-extrabold tracking-tight text-slate-100 font-sans">Trip Counting</h1>
      ...
      <div className="px-5 pt-5 space-y-3">
        ...
        {!isLoading && sessions.length === 0 && (
          <div className="mt-2 ios-glass-card p-6 text-center border-dashed">
            <div className="mx-auto h-12 w-12 rounded-full bg-blue-500/15 text-blue-400 grid place-items-center border border-blue-500/30">
              <Truck size={20} />
            </div>
        ...
        {sessions.map((s) => {
          ...
          return (
            <Link
              ...
              className="block ios-glass-card p-4 ios-press shadow-lg"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-mono text-sm font-bold uppercase tracking-wider truncate text-slate-100">
                    {s.title}
                  </p>
                  ...
                  {totalMinutes > 0 && (
                    <p className="text-[11px] text-slate-400 mt-1 flex items-center gap-1">
                      <Clock size={12} className="text-primary-glow" /> {totalMinutes} mins total loading time
                    </p>
                  )}
                </div>
                <div className="text-right shrink-0">
                  <p className="text-2xl font-black tabular-nums text-blue-400 leading-none font-mono">
                    {totalTyres}
                  </p>
