import type { LoadingSheetTrip } from "./types.ts";
...
export function formatWhatsAppShareText(entry: any, despatcherNameParam?: string): string {
  const despatcher = despatcherNameParam || entry?.despatcherName || "Theolus";
  ...
  lines.push("");
  lines.push(`🚛 *TRUCKS LOADED (${trips.length})*`);
  lines.push("----------------------------------");
  ...
  if (trips.length === 0) {
    ...
    trips.forEach((t, i) => {
      ...
      const startStr = formatTimeHHmm(t.startTime);
      const finishStr = formatTimeHHmm(t.finishTime);
      const durationMins = (t.durationMinutes && t.durationMinutes > 0)
        ? t.durationMinutes
        : calculateDurationMinutes(t.startTime, t.finishTime);

      lines.push(`${i + 1}. *${reg}* | Driver: ${driver} | ${tripId}`);
      lines.push(`   ⏱️ ${startStr} - ${finishStr} (${durationMins} mins) | 📦 ${t.quantityLoaded} tyres`);
      lines.push("");
    });
  }
